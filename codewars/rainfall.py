...
def mean(town, strng):
    ...
    for d in strng.split("\n"):
        found = re.findall(r"(\w+)(?=:)", d)[0]
        if found == town:
            city = found
            data = map(lambda x: float(x), re.findall(r"\d+\.\d", d))
            break
    if not city: return -1
    return sum(data) / 12
...
def variance(town, strng):
    ...
    for d in strng.split("\n"):
        ...
        if found == town:
            city = found
            data = list(map(lambda x: float(x), re.findall(r"\d+\.\d", d)))
            break
    if not city: return -1
    average = sum(data) / 12
    variance = 0
    for x in data:
        variance += pow(average-x, 2)
    return variance / 12 
